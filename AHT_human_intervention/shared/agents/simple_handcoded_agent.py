#!/usr/bin/env python3
"""
Simple Hand-Coded Agent - Pure hand-coded policy without human intervention
...
- NO human intervention capabilities - pure hand-coded policy
"""
...
class SimpleHandcodedAgent:
    """Simple Hand-Coded Agent without human intervention capabilities."""
    
    def __init__(self, mdp, agent_idx=0, agent_name="SimpleHandcoded"):
        self.mdp = mdp
    ...
    def _is_target_immediately_available(self, target_pos, state):
        """Check if a target is immediately available (not blocked by teammates)."""
        if state is None:
            return True
        
        # Check if any teammate is adjacent to the target
        for i, player in enumerate(state.players):
            if i != self.agent_idx:  # Don't check self
                if self._is_adjacent(player.position, target_pos):
                    return False
        return True
    
    def _get_closest_available_target(self, current_pos, targets, state=None):
        """Get closest AVAILABLE target - immediately checks for teammate blocking."""
        if not targets:
            return None
        
        # First, filter out targets that are immediately blocked by teammates
        available_targets = []
        for target in targets:
            if self._is_target_immediately_available(target, state):
                available_targets.append(target)
            else:
                print(f"[TARGET_BLOCKED] Target {target} immediately blocked by teammate")
        
        if not available_targets:
            print(f"[TARGET_BLOCKED] All targets immediately blocked, using original logic")
            return self._get_closest_target(current_pos, targets, state)
        
        # Among available targets, find the closest one
        best_target = None
        ...
        for target in available_targets:
            ...
            if adj_pos:
                if adj_pos == current_pos:
                    ...
                    path_len = len(path) if path is not None else float('inf')
    ...
    def _get_onion_with_astar(self, ego_pos, ego_orientation, state):
        """Get onion using A* pathfinding - finds closest AVAILABLE onion dispenser."""
        if not self.onion_locations:
            return 0
        
        # Check if current target is still available, if not, find a new one
        if (self.current_onion_target is None or 
            not self._is_target_immediately_available(self.current_onion_target, state)):
            self.current_onion_target = self._get_closest_available_target(ego_pos, self.onion_locations, state)
            if not self.current_onion_target:
                # Fallback to original logic if no available targets
        ...
        print(f"[SIMPLE] {self.agent_name} targeting AVAILABLE onion {target_onion}")
    ...
    def _put_onion_in_pot_with_astar(self, ego_pos, ego_orientation, state):
        """Put onion in pot using A* pathfinding - finds closest AVAILABLE pot."""
        if not self.pot_locations:
            return 0
        
        # Check if current target is still available, if not, find a new one
        if (self.current_pot_target is None or 
            not self._is_target_immediately_available(self.current_pot_target, state)):
            self.current_pot_target = self._get_closest_available_target(ego_pos, self.pot_locations, state)
        ...
        print(f"[SIMPLE] {self.agent_name} targeting AVAILABLE pot {target_pot}")
    ...
    def _get_dish_with_astar(self, ego_pos, ego_orientation, state):
        ...
        if self._is_adjacent(ego_pos, target_dish):
            if self._is_facing_target(ego_pos, ego_orientation, target_dish):
                print(f"[A*] {self.agent_name} facing dish dispenser {target_dish} - INTERACTING")
                # Reset target after successful interaction
                self.current_dish_target = None
                return 5  # INTERACT
            else:
    ...
    def get_action(self, state):
        """Get action using A* pathfinding - pure hand-coded policy."""
        self.step_count += 1
        ...
        try:
            ...
            print(f"[ERROR] Simple hand-coded agent {self.agent_idx} failed: {e}")
