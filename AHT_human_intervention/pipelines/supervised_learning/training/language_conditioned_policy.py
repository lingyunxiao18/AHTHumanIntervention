...
from overcooked_ai_py.mdp.overcooked_state_converter import state_to_text as describe_state
...
class TextBasedLangConditionedPolicy(nn.Module):
    ...
    def __init__(self, num_actions: int, text_dim: int = 768, hidden_dim: int = 256, 
        ...
        try:
            ...
            # Projection layer to reduce BERT output dimension if needed
            bert_output_dim = 768  # DistilBERT output dimension
            if text_dim != bert_output_dim:
                self.text_projection = nn.Linear(bert_output_dim, text_dim)
            else:
    ...
    def forward(self, mdp, states, human_commands: list = None) -> torch.Tensor:
        ...
        try:
            ...
            text_embeddings = bert_output.last_hidden_state[:, 0, :]  # Use CLS token [B, 768]
            
            # Project to desired text dimension
            text_embeddings = self.text_projection(text_embeddings)  # [B, text_dim]
    ...
    @classmethod
    def from_dir(cls, dir_path, ckpt_path=None):
        """Load a policy from a directory containing a checkpoint."""
        import os
        import torch
        
        if ckpt_path is None:
            # Look for .pt files in the directory
            ckpt_files = [f for f in os.listdir(dir_path) if f.endswith('.pt')]
            if not ckpt_files:
                raise ValueError(f"No checkpoint files found in {dir_path}")
            ckpt_path = os.path.join(dir_path, ckpt_files[0])
        ...
        # Create a new policy instance with the correct architecture
        # Determine the number of actions from the final policy head layer
        num_actions = None
        for key in state_dict.keys():
            if 'policy_head.6.weight' in key:  # Final layer
                num_actions = state_dict[key].shape[0]
                break
        
        if num_actions is None:
            # Default to 6 actions for Overcooked
            num_actions = 6
        
        # Get the architecture parameters from the checkpoint
        text_dim = checkpoint.get('text_dim', 256)
        hidden_dim = checkpoint.get('hidden_dim', 128)
        
        # Create policy with the correct architecture
        policy = cls(num_actions=num_actions, text_dim=text_dim, hidden_dim=hidden_dim)
