...
def retry_with_exponential_backoff(
    ...
    # Determine error classes compatible with installed OpenAI SDK
    if errors is None:
        try:
            from openai import RateLimitError, APIError, APIConnectionError, APITimeoutError, AuthenticationError  # type: ignore
            errors = (RateLimitError, APIError, APIConnectionError, APITimeoutError, AuthenticationError)
        except Exception:
            # Fallback to generic Exception if specific classes not available
            errors = (Exception,)
