...
class CrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        if self.config.humans.policy == 'orca' or self.config.humans.policy == 'social_force':
            ...
            self.max_human_num = config.sim.human_num + self.human_num_range
            assert config.sim.human_num >= self.human_num_range
            self.min_human_num = config.sim.human_num - self.human_num_range
    ...
    # teammate = True: the human has the same goal as the robot
    # teammate = False: the human goal is set in the opposite side of the circle
    def generate_circle_crossing_human(self, teammate=False):
        human = Human(self.config, 'humans')
        if self.randomize_attributes:
        ...
        if teammate:
            human.set(px, py, self.robot.gx, self.robot.gy, 0, 0, 0)
        else:
    ...
    def reset(self, phase='train', test_case=None):
        ...
        self.human_num = np.random.choice(self.min_human_num, self.max_human_num+1)
