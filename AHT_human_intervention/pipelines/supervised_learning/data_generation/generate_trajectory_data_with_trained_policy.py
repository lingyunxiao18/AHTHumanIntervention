#!/usr/bin/env python3
"""
Generate trajectory data with trained text policy as Policy0 and coordinated agent as Policy1.
This script generates trajectories using the partially trained policy stored in trained_policies/text_policy.
"""
...
class TrainedTextPolicyAgent:
    """Wrapper for the trained text policy to act as an agent."""
    
    def __init__(self, player_id: int, mdp, policy_path: str = "trained_policies/text_policy/text_policy.pt"):
        self.player_id = player_id
        self.mdp = mdp
        
        # Load the trained policy
        self.policy = TextBasedLangConditionedPolicy(num_actions=6, description_type="english", hidden_dim=128)
        ...
        if os.path.exists(policy_path):
            ...
            print(f"Warning: Policy file {policy_path} not found. Using untrained policy.")
    ...
    def action(self, state):
        """Get action from the trained text policy."""
        try:
            # Get action from the policy (no human command)
            action_idx = self.policy.get_action(self.mdp, state, human_command="", temperature=1.0)
            return action_idx
        except Exception as e:
            print(f"Error getting action from trained policy: {e}")
...
def create_agent_pairs(mdp: OvercookedGridworld) -> List[Tuple[str, AgentPair]]:
    """Create agent pairs with trained text policy as Policy0 and coordinated agent as Policy1."""
    
    # Create all available partner agents (Policy1)
    partner_agents = {
    ...
    # Create pairs with trained text policy as ego agent (agent 0)
    for partner_name, partner_agent in partner_agents.items():
        pair_name = f"trained_text_policy_vs_{partner_name}"
        
        # Create new instances
        ego_agent = TrainedTextPolicyAgent(0, mdp)
...
def generate_trajectories_for_pair(env: OvercookedEnv, agent_pair: AgentPair, 
                                 pair_name: str, num_trajectories: int = 100, 
    ...
    for traj_idx in tqdm(range(num_trajectories), desc=pair_name):
        ...
        while not done and step < horizon:
            ...
            state_text = describe_state(env.mdp, state, mode="english")
            ...
            try:
                ...
                # Execute joint action - ensure both actions are valid
                if len(env_actions) == 2:
                    state, reward, done, info = env.step(env_actions)
                else:
                    print(f"Warning: Expected 2 actions, got {len(env_actions)}")
                    break
...
def main():
    ...
    # Configuration
    num_trajectories_per_pair = 10  # Reduced for testing
    horizon = 200
    layout_name = "random3"  # You can change this to other layouts
    
    # Create output directory
    output_dir = "generated_data/trajectories_trained_policy"
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"Generating trajectory data with trained text policy as Policy0")
    print(f"Number of trajectories per pair: {num_trajectories_per_pair}")
    ...
    metadata = {
        ...
        'description': 'Trajectory data with trained text policy as Policy0 and coordinated agent as Policy1, no human commands'
    }
    ...
    if all_trajectories:
        ...
        print("No trajectories were generated successfully.")
