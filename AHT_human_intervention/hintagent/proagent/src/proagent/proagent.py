...
# Try to import helpers; fallback to stubs if not available in shared search
try:
    from shared.envs.envs.overcooked.overcooked_ai_py.planning.search import get_intersect_counter, query_counter_states  # type: ignore
except Exception:
...
class ProMediumLevelAgent(ProAgent):
	...
	def __init__(
		...
		# Debug/telemetry: last planner LLM response
	...
	def create_gptmodule(self, module_name, file_type='txt', retrival_method='recent_k', K=10):
		...
		if module_name == "planner":
			...
			# Fallbacks if agent-specific file is missing
			if not os.path.exists(prompt_file):
				alt1 = os.path.join(PROMPT_DIR, model_name, module_name, self.prompt_level, f'{self.layout}.{file_type}')
				alt2 = os.path.join(PROMPT_DIR, model_name, module_name, self.prompt_level, 'default.txt')
				prompt_file = alt1 if os.path.exists(alt1) else (alt2 if os.path.exists(alt2) else prompt_file)
		elif module_name == "explainer":
			prompt_file = os.path.join(PROMPT_DIR, model_name, module_name, f'player{self.agent_index}.{file_type}')
			if not os.path.exists(prompt_file):
				alt = os.path.join(PROMPT_DIR, model_name, module_name, 'player.txt')
				prompt_file = alt if os.path.exists(alt) else prompt_file
		else:
