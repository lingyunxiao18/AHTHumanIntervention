...
from shared.envs.envs.overcooked.overcooked_ai_py.agents.agent import GreedyHumanModel, StayAgent, RandomAgent
from shared.envs.envs.overcooked.overcooked_ai_py.agents.agent import AgentFromPolicy, AgentPair
from shared.envs.envs.overcooked.overcooked_ai_py.planning.planners import MediumLevelPlanner, NO_COUNTERS_PARAMS
from shared.envs.envs.overcooked.overcooked_ai_py.utils import load_dict_from_file, load_pickle



# ProAgent is only needed when alg == "ProAgent"; import lazily in make_agent

from collections import defaultdict
## Note: Heavy dependencies (tensorflow, stable_baselines) are imported lazily
...
NEW_LAYOUTS = {
    # Map to shared layouts available in repo
    "cramped_room": "simple",
    "coordination_ring": "random1",
    "forced_coordination": "random0",
    "counter_circuit": "random3",
...
def _adapt_mdp_get_state_transition_to_three_tuple(mdp):
    """Ensure mdp.get_state_transition returns 3-tuple for shared library compatibility."""
    import types
    orig_fn = mdp.get_state_transition
    def wrapper(self, state, joint_action):
        result = orig_fn(state, joint_action)
        # Shared mdp may return (next_state, mdp_infos)
        if isinstance(result, tuple) and len(result) == 2:
            next_state, _mdp_infos = result
            # Provide dummy sparse and dense rewards for callers expecting 3 values
            return next_state, 0, 0
        return result
    mdp.get_state_transition = types.MethodType(wrapper, mdp)
    ...
    if alg == "Stay":
        ...
        if alg == "ProAgent":
            ...
            try:
                ...
                from proagent.proagent import ProMediumLevelAgent  # type: ignore
...
def get_model_policy_from_saved_model(save_dir, sim_threads):
    ...
    # Lazy import TensorFlow only when needed for loading saved models
    import tensorflow as tf  # noqa: WPS433
    from tensorflow.python.saved_model import tag_constants  # noqa: WPS433
...
def get_model_policy(step_fn, sim_threads, is_joint_action=False):
    ...
    def state_policy(mdp_state, mdp, agent_index, stochastic=True, return_action_probs=False):
        ...
        # Avoid hard dependency on TensorFlow when not installed
        try:
            import tensorflow as tf  # noqa: WPS433
            is_tf_tensor = isinstance(action_probs, tf.Tensor)
        except Exception:
            is_tf_tensor = False
        if is_tf_tensor:
...
def load_bc_model_from_path(model_name):
    ...
    # Lazy import stable_baselines only when loading BC models
    from stable_baselines import GAIL  # noqa: WPS433
